...
export async function register(email: string, password: string) {
  const res = await fetch('/chronos/api/auth/register', {
    method: 'POST',
...
export async function login(email: string, password: string): Promise<AuthResult> {
  const res = await fetch('/chronos/api/auth/login', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email, password })
  });
...
export async function createTodo(payload: { title: string; description?: string; date: string; time?: string; groupId: string; subtasks?: { title: string; completed?: boolean }[] }, token: string) {
  const res = await fetch('/chronos/api/todos', {
    method: 'POST',
    headers: authHeaders(token),
...
export async function deleteTodo(id: string, token: string) {
  const res = await fetch(`/chronos/api/todos?id=${encodeURIComponent(id)}`, {
    method: 'DELETE',
    headers: authHeaders(token)
  });
  let data: any = null;
...
export async function getCalendar(month: string, token: string) {
  const res = await fetch(`/chronos/api/calendar?month=${encodeURIComponent(month)}`, {
    headers: authHeaders(token)
  });
  let data: any = null;
  try { data = await res.json(); } catch {}
...
export async function toggleSubtask(id: string, token: string) {
  const res = await fetch('/chronos/api/subtasks', {
    method: 'PATCH',
    headers: authHeaders(token),
    body: JSON.stringify({ id })
  });
